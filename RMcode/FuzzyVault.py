...
class Vault:
    ...
    def unlock(self, ints):
        if self.attempts_count>0:
            ...
            dec1=self.splited_unlock(attempt)
            print(dec, dec1)
            if (dec==self.secret):
                print("Unlocked!!!")
            else:
    ...
    def splited_unlock(self, y):
        ...
        y_L_corrected1 = (y_L + v_hat_codeword) % 2
        ...
        codeword1_right = (u_hat_codeword1 + v_hat_codeword) % 2
        codeword1 = np.concatenate((codeword1_left, codeword1_right))
        codeword2_left = u_hat_codeword2
        codeword2_right = (u_hat_codeword2 + v_hat_codeword) % 2
