...
def _binom(m: int, k: int) -> int:
    """
    Compute C(m, k) using math.comb with LRU caching.
    """
...
def _xor_all(arr: List[int]) -> int:
    """
    XOR all elements in a list of integers.
    """
    x = 0
    for v in arr:
        x ^= v
...
def _majority(arr: List[int]) -> int:
    """
    Return the majority bit in arr (tie breaks to 1).
    """
    zeros = arr.count(0)
...
def _all_combos(m: int, degree: int) -> List[Tuple[int, ...]]:
    """
    Return all combinations of `degree` indices from range(m), cached globally.
    """
    key = (m, degree)
...
def _generation_g1(m: int) -> np.ndarray:
    """
    Generate the degree-1 Reed-Muller generator rows (variables x_i patterns).
    """
    n = 1 << m
...
def _generation_gr(m: int, degree: int) -> np.ndarray:
    """
    Generate generator rows for monomials of a given degree using bitwise AND reduction.
    """
    g1 = _generation_g1(m)
...
def _get_gr(m: int, degree: int) -> np.ndarray:
    """
    Get (or build and cache) generator rows for a given degree.
    """
    key = (m, degree)
...
def _get_matrix(m: int, r: int) -> np.ndarray:
    """
    Get (or build and cache) the full generator matrix for RM(m, r).
    """
    key = (m, r)
    if key not in _matrix_cache:
        rows = [np.ones((1, 1 << m), dtype=np.uint8), _generation_g1(m)]
...
if _HAS_NUMBA:
    ...
    def _decode_block_jit(
        ...
        """
        Numba-accelerated block decoder for the highest-degree coefficients.
        """
        ...
        for row in range(n_rows):
            ...
            result[row] = 0 if zeros > ones else 1
...
class CombinedRM:
    """
    Reed-Muller RM(m, r) encoder/decoder with majority-logic decoding.
    Uses cached generator matrices and optional numba acceleration.
    """

    def __init__(self, m: int, r: int) -> None:
        """
        Initialize RM(m, r) parameters and warm up internal caches.
        """
        if not (0 <= r <= m):
    ...
    def _warm(self) -> None:
        """
        Precompute and cache frequently used matrices and combinations.
        """
    ...
    def encode(self, message: List[int] | np.ndarray) -> np.ndarray:
        """
        Encode a message (length k) into a codeword (length n).
        """
    ...
    def _decode_highest_degree_block(
        self,
        block_len: int,
        encoded: np.ndarray,
        degree: int,
        ...
        """
        Decode coefficients of the highest degree block using majority logic.
        """
        if _HAS_NUMBA:
    ...
    def decode_without_erasures(self, word: np.ndarray | List[int]) -> List[int]:
        """
        Decode a received word without erasures into the original message coefficients.
        """
    ...
    def decode(self, word: np.ndarray | List[int]) -> List[int]:
        """
        Decode a word (length n) into a message (length k).
        """
...
def _get_multipliers(num_mult: int, num_x: int, idx: int) -> Tuple[int, ...]:
    """
    Return the idx-th combination of size num_mult from range(num_x).
    Uses caching for small combination sets and islice for large ones.
    """
